...
grafico1<-ggplot(data=ds,aes(x = MinTemp, y = MaxTemp))+geom_point()
grafico2<-ggplot(data=ds,aes(x = MinTemp, y = MaxTemp,color=RainTomorrow))+geom_point()
# grafico2+scale_color_manual(values=c("blue","green"))
grafico3<-grafico2+ggtitle("Gráfica ds MaxTemp y MinTemp")
...
library(ggplot2)
load("data/weatherAUS.RData")
ggplot(data=ds,aes(x = Temp9am, y = Pressure9am,shape=RainToday))+geom_point() +labs(title = "Representación de la presión y la temperatura",
       subtitle = "Relación entre la presión y la temperatura a las 9 am",
       x = "Temperatura 9am",
       y = "Presión 9am",
       caption = "Presión Temperatura 9am")
ggplot(data=ds,aes(x = Temp9am, y = Pressure9am,color=RainToday))+geom_point() +labs(title = "Representación de la presión y la temperatura",
       subtitle = "Relación entre la presión y la temperatura a las 9 am",
       x = "Temperatura 9am",
       y = "Presión 9am",
       caption = "Presión Temperatura 9am")
...
```
Se observa mejor la información que introduce la variable ``RainToday``con la representación con los colores que con la forma.
...
# iris.grafico1<-ggplot(data=iris.tidy,aes(x=Species,y=Value))+geom_boxplot()
# 
# iris.grafico2<-ggplot(data=iris.tidy,aes(x=Part,y=Value,color=Part))+geom_boxplot()
# 
# iris.grafico3<-iris.grafico2+facet_grid(rows = vars(Measure))

iris.grafico1<-ggplot(data=iris.tidy,aes(x=Species,y=Value))+geom_point()
iris.grafico1
iris.grafico2 <- ggplot(data = iris.tidy, aes(x = Species, y = Value, color = Part)) +geom_point()
iris.grafico2
iris.grafico3<-ggplot(data=iris.tidy,aes(x=Part,y=Value,color=Part))+geom_point()
iris.grafico3

iris.grafico4_filas<-iris.grafico2 + facet_grid(rows = vars(Measure))
iris.grafico4_filas
iris.grafico4_columnas<-iris.grafico2 + facet_grid(cols = vars(Measure))
iris.grafico4_columnas
# Añadimos un poco de ruido aleatorio para observar mejor la densidad de puntos en cada parte
iris.grafico5<-ggplot(data=iris.tidy,aes(x=Species,y=Value,color=Part))+geom_jitter()+facet_grid(cols = vars(Measure))
iris.grafico5
...
plot1<-ggplot(data=DatosEjer4,aes(x=Fecha,y=Ventas,color=Fabricante))+geom_point()
plot1
# Este gráfico no es muy bueno para evaluar la evolución de las ventas de cada fabricante por los puntos, es mejor con geom_line
plot2<-plot1+geom_line()
plot2
plot3<-ggplot(data=DatosEjer4,aes(x=Fecha,y=Ventas,color=Fabricante))+geom_point()+geom_line(aes(linetype=Fabricante))

plot3+ggtitle("Ventas en el tiempo de distintos fabricantes")
...
plot1<-ggplot(data=Experimento,aes(x=Dosis,y=Altura,group = Complemento,linetype = Complemento,shape = Complemento))+geom_line()+geom_point()
plot1

plot2<-plot1+  geom_errorbar(aes(ymin = Altura - Error_Altura, ymax = Altura + Error_Altura)) +labs(x = "Dosis", y = "Altura", title = "Altura en función de la dosis con barras de error")
plot2

plot3<-ggplot(data = Experimento, aes(x = Dosis, y = Altura, color = Complemento))+geom_line(aes(group = Complemento),linetype ="dashed")+scale_color_brewer(palette = "Reds")
plot3



ggsave("figs/Ejercicio5.pdf")
...
# ?tseries::ice.river
# CUIDADO: TIME NO ES UNA VARIABLE DE TIPO TIEMPO (ES FLOAT), PERO ESTÁ BIEN PUESTO PARA QUE NO HAYA QUE PASARLO A ESE FORMATO



# ggplot(data=ice.river.df,aes(x=time,y=flow.vat))+geom_point(color="red")
# ggplot(data=ice.river.df,aes(x=time,y=flow.jok))+geom_point(color="blue")

ggplot(data=ice.river.df,aes(x=time))+geom_line(aes(y=flow.vat),color="red")+geom_line(aes(y=flow.jok),color="blue")+labs(title="Caudales de los ríos islandeses Vatnsdalsa (azul) y Jukulsa Eystri (rojo)",y=TeX("Caudal $(m^3/s)$"),caption="Fuente")




ggsave("figs/Ejercicio6.pdf")
...
plot1<-ggplot(data=ds,aes(x=Year,y=MinTemp))+geom_boxplot()
plot1
plot2<-ggplot(data=ds,aes(x=Year,y=MaxTemp))+geom_violin(aes(fill=Year))+labs(title = "Distribución de temperaturas máximas por año",x="Año",y=TeX("Temperatura $ (ºC)$"))+theme(axis.text.x = element_text(angle = 45, hjust = 1))
plot2

ggsave("figs/Ejercicio7.pdf")
...
chol <- read_table(url)
  ...
  # # Usamos after_stat(density) o ..density.. (sintaxis antigua) para mapear Y a la densidad
  # geom_histogram(aes(y = after_stat(density)), breaks = seq(155, 195, by = 2), alpha = 0.5, color = "red", fill = "pink") +
  # geom_density(color = "green", linewidth = 1) + # El ancho de línea (size) es mejor con linewidth
  # coord_cartesian(xlim = c(155, 195)) +
  # labs(title = "Histograma y Densidad de la Altura (Escala de Densidad)",
  #      x = "Altura (cm)",
  #      y = "Densidad")



plot1<-ggplot(data=chol,aes(x=HEIGHT))+geom_histogram(aes(y = after_stat(density)), breaks = seq(155, 195, by = 2), alpha = 0.5, color = "red")+geom_density(color="green")+coord_cartesian(xlim = c(155, 195)) +labs(title="Histograma de la variable Altura",x="Altura (cm)",y="Frecuencia")
plot1

plot2<-plot1+facet_grid(rows=vars(MORT))
plot2

ggsave("figs/Ejercicio8.pdf")
